...
Chức năng: 
 - Lưu trạng thái của trò chơi cờ vua.
 - In bảng cờ để hiển thị trạng thái hiện tại của trò chơi.
 - Tìm các nước đi hợp lệ cho từng quân cờ trên bảng cờ.
 - Lưu nhật ký nước đi để theo dõi lịch sử các bước di chuyển trong trò chơi.
"""
from Piece import Rook, Knight, Bishop, Queen, King, Pawn
from enums import Player

# Bảng mô tả vị trí của các ô trên bàn cờ
'''
r \ c     0           1           2           3           4           5           6           7 
...
# TODO: Cập nhật bàn cờ theo người chơi
# TODO: Quân tốt thường được biểu diễn bằng không có chữ
# TODO: tình trạng hòa cờ
# TODO: lưu trạng thái di chuyển - sửa lỗi cập nhật kiểu boolean của cờ lâu vua
# TODO: thay đổi đối số phương thức di chuyển về is_ai thành một cái gì đó tinh tế hơn
class game_state:
    """
        Khởi tạo bàn cờ với mảng 2 chiều
    """
    def __init__(self):
        self.white_captives = []  # Quân trắng bị bắt
        self.black_captives = []  # Quân đen bị bắt
        self.move_log = []  # Lịch sử các bước đi
        self.white_turn = True  # Lượt của người chơi trắng
        self.can_en_passant_bool = False  # Cờ En passant
        self._en_passant_previous = (-1, -1)  # Vị trí trước đó của quân en passant
        self.checkmate = False  # Kiểm tra tình trạng chiếu mát
        self.stalemate = False  # Kiểm tra tình trạng hòa cờ

        self._is_check = False  # Kiểm tra tình trạng chiếu
        self._white_king_location = [0, 3]  # Vị trí của vua trắng
        self._black_king_location = [7, 3]  # Vị trí của vua đen

        self.white_king_can_castle = [True, True, True]  # Kiểm tra có thể phong vị cho vua trắng
        self.black_king_can_castle = [True, True, True]  # Kiểm tra có thể phong vị cho vua đen
    ...
    def get_piece(self, row, col):
        """
        Lấy thông tin về quân cờ ở vị trí (row, col) trên bàn cờ.

        Parameters:
        - row (int): Số thứ tự của hàng.
        - col (int): Số thứ tự của cột.

        Returns:
        - Quân cờ hoặc giá trị rỗng nếu không có quân cờ ở vị trí đó.
        """
    ...
    def is_valid_piece(self, row, col):
        """
        Kiểm tra xem vị trí (row, col) có chứa quân cờ hợp lệ hay không.
        ...
        Returns:
        - True nếu là quân cờ hợp lệ, False nếu không phải.
        """
    ...
    def get_valid_moves(self, starting_square):
        """
        Lấy danh sách các bước đi hợp lệ cho quân cờ ở vị trí đầu tiên.

        Parameters:
        - starting_square (tuple): Tọa độ (row, col) của quân cờ đầu tiên.

        Returns:
        - Danh sách các bước đi hợp lệ cho quân cờ đó.
        """
        current_row = starting_square[0]
        current_col = starting_square[1]

        # Kiểm tra xem vị trí xuất phát có chứa quân cờ hợp lệ không
        if self.is_valid_piece(current_row, current_col):
            valid_moves = []
            moving_piece = self.get_piece(current_row, current_col)
            
            # Xác định vị trí vua của người chơi đang di chuyển
            ...
            # Kiểm tra tình trạng chiếu và các quân đang bị chiếu, bị kẹt, và kiểm tra nước đi hợp lệ ban đầu    
            ...
            # Kiểm tra trường hợp chiếu ngay lập tức
            if checking_pieces:
                for move in initial_valid_piece_moves:
                    ...
                    for piece in checking_pieces:
                        if moving_piece.get_name() == "k":
                            ...
                            # Kiểm tra xem di chuyển có loại bỏ chiếu không
                        ...
                        # Xử lý trường hợp nước đi là quân cờ chiếu và không bị kẹt    
                        elif move == piece and len(checking_pieces) == 1 and moving_piece.get_name() != "k" and \
                                (current_row, current_col) not in pinned_pieces:
                            pass
                        # Xử lý trường hợp nước đi không phải quân cờ chiếu và không bị kẹt
                        elif move != piece and len(checking_pieces) == 1 and moving_piece.get_name() != "k" and \
                                (current_row, current_col) not in pinned_pieces:
    ...
    def check_for_check(self, king_location, player):
        '''
        Kiểm tra vị trí của vua có nguy cơ bị chiếu ngay lập tức

        - Kiểm tra 8 hướng và 8 ô đi của quân mã để xác định có tình trạng chiếu không
        - Kiểm tra xem có tình trạng "pin" (khi có quân chặn giữ) không
        - Nếu có tình trạng chiếu, ghi lại vị trí chiếu
        - Nếu có tình trạng "pin", ghi lại vị trí "pin" và vị trí chiếu liên quan

        Đầu vào:
            - king_location: tuple chứa vị trí của vua (dòng, cột)
            - player: Biểu diễn bên cần kiểm tra (Player.WHITE hoặc Player.BLACK)

        Đầu ra:
            - Danh sách gồm 3 phần tử:
                - Danh sách các vị trí bị chiếu ngay lập tức
                - Danh sách các vị trí bị "pin"
                - Danh sách các vị trí bị "pin" và chiếu liên quan
        '''
        _checks = []
        _pins = []
        _pins_check = []
